...
combined_data, combined_mean, combined_std = normalize_dataset(load_dataset("world"), (0, 1))
ukraine_data, ukraine_mean, ukraine_std = normalize_dataset(load_dataset("ukraine"))

window = 14
train, test = create_train_test_datasets(combined_data, ukraine_data, x_window=window)


checkpoint = ModelCheckpoint("best_combined_model.hdf5", verbose=1, save_best_only=True, mode='auto', period=1)
callback = EarlyStopping(patience=10, restore_best_weights=True)
model = create_deep_model(combined_data.shape[1], 3)
